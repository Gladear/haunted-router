...
function createRouteEntry<T>([path, callback]: [string, RouteCallback<T>]): Route<T> {
  let pattern = '^', lastIndex = 0, match: RegExpExecArray | null;
  ...
  const matcher = (path: string) => {
    ...
    const params = names.reduce((obj, name, i) => ({
      ...obj,
      [name]: values[i],
    }), {});
...
const useRoutes = hook(class<T> extends Hook<[Routes<T>, T], T, Element> {
  fallback: T;
  _routes: Route<T>[];
  _result!: T;

  constructor(id: number, state: State<Element>, routes: Routes<T>, fallback: T) {
    super(id, state);
